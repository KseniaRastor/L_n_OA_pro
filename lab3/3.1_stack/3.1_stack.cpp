...
struct node {
	...
	struct node* next, *prev;									// ссылка на соседние элементы (следующий и предыдущий)
};
typedef node* pnode;											// тип "указатели на узел"

struct stack {
	pnode head, tail;											//голова и хвост стека
};
...
stack* push(stack &S, char &name) {
		...
		if (S.head) S.head->prev = newnode;	//если в стеке был элемент, то он становится предыдущим для головы
...
void review(stack &S) {
	...
	while(1){
		printf("Имя - %s, \n", S_tmp.head->inf);	//вывод поля inf текущего узла
		if (S_tmp.head->next == NULL) {				//если указатель на следующий элемент NULL, дошли до конца списка
			return;
		}
		else {
			S_tmp.head = S_tmp.head->next;			//переход на следующий узел
		}
	}
}
...
void find(stack &S, char &name) {
	...
	do {
		if (strcmp(&name, S_tmp.head->inf) == 0) {	//найдено совпадение - вывод номера элемента в стеке
			printf("Элемент №%d в списке\n", i);
			return;
		}
		i++;
		S_tmp.head = S_tmp.head->next;			//переход к следующему узлу
...
stack* pop(stack &S) {
	...
	pnode TopNode = S.head;				//TopNode указывает на первый узел стека
	S.head = TopNode->next;				//головой стека становится второй узел
	if (S.head) S.head->prev = NULL;	//указатель на предыдущий узел для второго узла ссылается на NULL
	else S.tail = NULL;					//если S.head == NULL, то список пуст (состоял из одного удаляемого узла)

	delete TopNode;						//удаление 
